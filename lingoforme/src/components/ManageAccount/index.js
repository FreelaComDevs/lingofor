...
class ManageAccount extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dialog: {
        ...
        message: "",
        onClose: () =>
          console.log("You should implement the onClose function."),
      },
    };
  ...
  componentWillMount() {
    ...
    this.setState(
      { id: user.id, role: user.role, languageLabel: languageLabel },
      () => {
        if (this.state.id > 0) {
          this.callApi(false);
        }
  ...
  callApi(showModal) {
    if (!showModal) {
      ...
      this.service
        .get("languages?skip=0&take=999999")
        .then((res) => {
          this.setState({
    ...
    this.service
      ...
      .then((res) => {
        ...
        if (this.state.role === "student") {
          if (
            res.result.items[0].students &&
            res.result.items[0].students.length > 0
          ) {
            res.result.items[0].languages =
              res.result.items[0].students[0].studentLanguages;
            res.result.items[0].classTool =
              res.result.items[0].students[0].classTool;
            res.result.items[0].classToolId =
              res.result.items[0].students[0].classToolId;
          }
        } else if (this.state.role === "teacher") {
          if (
            res.result.items[0].teachers &&
            res.result.items[0].teachers.length > 0
          )
            res.result.items[0].languages =
              res.result.items[0].teachers[0].teacherLanguages;
        } else if (this.state.role === "coordinator") {
          if (
            res.result.items[0].coordinators &&
            res.result.items[0].coordinators.length > 0
          ) {
            res.result.items[0].languages =
              res.result.items[0].coordinators[0].coordinatorLanguages;
            res.result.items[0].coordinatorLanguageResponsible =
              res.result.items[0].coordinators[0].coordinatorLanguageResponsibles;
          }
        }
        ...
        if (
          ...
          if (
            res.result.items[0].languages &&
            res.result.items[0].languages.length > 0
          ) {
            res.result.items[0].languages.sort((x, y) =>
              x.isNative === y.isNative ? 0 : x.isNative ? -1 : 1
            );
          } else if (
            res.result.items[0].languages &&
            res.result.items[0].languages.length === 0
        ...
        if (showModal) {
          this.setState(
            {
              ...res.result.items[0],
              originalTimezone: res.result.items[0].timezone,
              loading: false,
              openalert: true,
              alerttitle: this.t("SAVE_FORM_TITLE"),
              alertdescription: this.t("SAVE_FORM_DESCRIPTION"),
            },
            () => this.validateForm()
          );
        } else {
  ...
  handleUploadFile(e) {
    if (e.target.files[0]) {
      ...
      if (file && file !== this.state.oldpicture) {
        ...
        this.service
          .ApiPosts(`admin/users/${this.state.id}/picture`, pictureData)
          .then((res) => {
  ...
  validateForm() {
    ...
    validations = {
      ...
      birthday:
        ...
        moment(this.state.birthDate) >= moment().add(-1, "hours")
          ? true
          : false,
      gender: validator.isEmpty(this.state.gender),
      phone:
        this.state.userPhones.length === 0 ||
        validator.isEmpty(this.state.userPhones[0].phone),
    };
    if (this.state.role === "student")
      validations.nativelanguage =
        this.state.languages.length === 0 ||
        this.state.languages[0].languageId === 0
          ? true
          : false;
  ...
  handleSubmit(e) {
    ...
    if (Object.values(this.state.validations).indexOf(true) < 0) {
      ...
      this.service
        .ApiPut(urlCall, JSON.parse(JSON.stringify(formData)))
        .then((res) => {
          if (!res || !res.success) {
            return;
          }
        ...
        .catch((e) => {
          ...
          this.setState({
            ...
            alertdescription: this.t("ERROR_DIALOG"),
          });
        });
    } else {
      this.setState({
        openalert: true,
        loading: false,
        alerttitle: this.t("INVALID_FORM_TITLE"),
        alertdescription: this.t("INVALID_FORM_DESCRIPTION"),
      });
    }
  }
  ...
  submitPassword(e) {
    ...
    this.service
      .ApiPosts("admin/users/changepassword", formData)
      .then((res) => {
  ...
  render() {
    ...
    const nativeLanguage = this.state.languages.find((item) => item.isNative);
    const validationLanguageNative = nativeLanguage
    ...
    return (
      <div className="view">
        ...
        <Dialog
          ...
          className="boxModal"
        >
          <DialogTitle id="pass-dialog-title">
            <span className="titleModal">{t("CHANGE_PASSWORD_TITLE")}</span>
          </DialogTitle>
          <DialogContent>
            <div id="pass-dialog-description" className="boxModal">
              <form className="formulario">
                ...
                <input
                  type="password"
                  placeholder={this.t("CHECK_PASS_PLACEHOLDER")}
              ...
              <div>
                ...
                >
                  <DialogTitle id="alert-dialog-title">
                    <span className="titleModal">{this.state.alerttitle}</span>
                  </DialogTitle>
                  <DialogContent>
                    <DialogContentText
                      id="pass-dialog-description"
                      className="boxModal"
                    >
                      <form className="formulario">
                        <div className="description">
                  ...
                  <div className="box">
                    ...
                    <form className="formulario">
                      <div>
                        <label
                          htmlFor="name"
                          required={this.state.validations["name"]}
                        >
                          {this.t("NAME")}
                        </label>
                        <span className="invalid">
                          {this.state.validations["name"] &&
                            this.t("REQUIRED_FIELD")}
                        </span>
                      </div>
                      <input
                        type="text"
                              ...
                              <div className="switchBox">
                                <div className="switch__container">
                                  <input
                                    ...
                                    checked={item.notify}
                                  />
                                  <label htmlFor={"notify" + index}>
                                    <span>{this.t("NOTIFICATION")}</span>
                                  </label>
                                </div>
                              </div>
                              <button
                      ...
                      <div className="inputs">
                        ...
                        <div className="lineInputs">
                          <div>
                            ...
                            <span className="invalid">
                              {this.state.validations["birthday"] &&
                                this.t("REQUIRED_FIELD")}
                            </span>
                          </div>
                        ...
                        <div>
                          <div>
                            <label htmlFor="language">
                              {this.t("NATIVE_LANGUAGE")}
                            </label>
                            <span className="invalid">
                              {this.state.validations["nativelanguage"] &&
                                this.t("REQUIRED_FIELD")}
                            </span>
                          </div>
                          <div>
                            <select
                              name="language"
                              value={
                                validationLanguageNative
                                  ? validationLanguageNative
                                  : ""
                              }
                              disabled={
                                this.state.role !== "student" ? true : null
                              }
                      ...
                      {(this.state.role === "student" ||
                        ...
                        this.state.languages
                          ...
                          .map((item, index) => {
                            if (
                              this.state.role === "student" ||
                              (index >= 0 && this.state.role !== "student")
                            )
                              index = index + 1;
                            return (
                              <div key={index}>
                                ...
                                <div>
                                  ...
                                  >
                                    ...
                                    {this.state.globallanguages.map(
                                      (item, i) => {
                                        i = i + 1;
                                        return (
                                          <option
                      ...
                      {this.state.role === "coordinator" &&
                        this.state.coordinatorLanguageResponsible &&
                        this.state.coordinatorLanguageResponsible.length >
                          0 && (
                          <div>
                            <div>
                              <label htmlFor={"lingoLanguage"}>
                                {this.t("COORDINATED_LANGUAGES")}
                              </label>
                            </div>
                            <div>
                              <select
                                ...
                                className="language inputMobile"
                              >
                                {this.state.coordinatorLanguageResponsible.map(
                                  (lang) => {
                                    return (
                                      <option
                                        key={"lingoLang" + lang.id}
                                        value={lang.id}
                                      >
                                        {lang.lingoLanguage.desc}
                                      </option>
                                    );
                                  }
                                )}
                      ...
                      <div className="inputs">
                        ...
                        <div className="lineInputs">
                          <div>
                            ...
                            <span className="invalid">
                              {this.state.validations["timezone"] &&
                                this.t("REQUIRED_FIELD")}
                            </span>
                          </div>
                          <TimezonePicker
                        ...
                        <div className="lineInputs">
                          ...
                          <p>
                            {this.t("CURRENT_TIME")}
                            <br /> {this.t("IN_THIS_TIMEZONE")}: {dataTimeZone}
                          </p>
                        </div>
                      </div>
                      {this.state.firstAccess == false &&
                        this.state.role === "student" && (
                          <div className="inputs selectClass">
                            ...
                            <div className="lineInputs">
                              <div>
                                <label htmlFor="classToolId">
                                  {this.t("TOOL_ID")}
                                </label>
                                <span className="invalid">
                                  {this.state.validations["classToolId"]}
                                </span>
                              </div>
                              <span></span>
                              <input
                      ...
                      {this.state.userPhones.map((item, index) => {
                        return (
                          ...
                          >
                            ...
                            <div className="lineInputs">
                              <div>
                                <label htmlFor={"userPhoneTypeId" + index}>
                                  {this.t("TYPE")}
                                </label>
                              </div>
                              <select
                            ...
                            {index === 0 && (
                              <div className="lineInputs">
                                <button
                                  name="addphone"
                                  className="addInput inputMobile"
                                  onClick={this.addField}
                                >
                                  {this.t("BTN_ADD_TEL")} +
                                </button>
...
export default connect(mapStateToProps)(
  translate("translations")(ManageAccount)
);
