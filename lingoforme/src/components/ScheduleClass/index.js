...
class Schedules extends Component {
  ...
  componentDidMount() {
    this.indexkey =
      Idioma.language === "pt"
        ...
        : "en_US";
    this.serv
      .get(`studentplans/getbystudentid/${this.state.studentId}`)
      .then((res) => {
        let planos = [];
        let lingoLanguages = [];
        let languages = [];
        ...
        this.setState({
          scheduledEnd:
            this.state.timeSlots[
              this.state.scheduledStartSelectedIndex + parseInt(e.target.value)
            ].start,
        });
      }
    }
  ...
  submitHandle = () => {
    ...
    if (this.state.showRepeat) {
      submitObj = {
        ...
        scheduledStart:
          this.state.scheduledDate + "T" + this.state.scheduledStart + ".000",
        scheduledEnd:
          this.state.scheduledDate + "T" + this.state.scheduledEnd + ".000",
      ...
      this.serv
        .ApiPosts(urlPostSchedule, submitObj)
        .then((res) => {
          ...
          this.setState({
            ...
            alertMsg:
              '<div className="container"><br> <img src="' +
              successImg +
              '"/> <br> ' +
              this.props.t("BTN_SCHEDULE_ADDED") +
              "</div>",
          });
          if (this.props.toggleShowing) {
        ...
        .catch((err) => {
          ...
          if (
            err.data.error !== null &&
            (err.data.error.httpCode === "400" ||
              err.data.error.httpCode === "500")
          ) {
  ...
  validateForm = () => {
    if (
      this.state.lingoLanguageId === "" ||
      this.state.lingoLanguageId === null ||
      this.state.lingoLanguageId === undefined ||
      this.state.lingoLanguageId === 0
    ) {
      this.setState({
        loading: false,
        alertTitle: this.props.t("ERROR_MESSAGE"),
    ...
    if (
      this.state.studentPlanId === "" ||
      this.state.studentPlanId === null ||
      this.state.studentPlanId === undefined
    ) {
      this.setState({
        loading: false,
        alertTitle: this.props.t("ERROR_MESSAGE"),
    ...
    if (
      ...
      this.state.scheduledDate === null ||
      this.state.scheduledDate === undefined ||
      this.state.scheduledDate === 0
    ) {
      this.setState({
        loading: false,
        alertTitle: this.props.t("ERROR_MESSAGE"),
    ...
    if (
      ...
      this.state.scheduledStart === null ||
      this.state.scheduledStart === undefined ||
      this.state.scheduledStart === 0
    ) {
      this.setState({
        loading: false,
        alertTitle: this.props.t("ERROR_MESSAGE"),
    ...
    if (
      this.state.numberOfClasses === "" ||
      this.state.numberOfClasses === null ||
      this.state.numberOfClasses === undefined ||
      this.state.numberOfClasses === 0
    ) {
      this.setState({
        loading: false,
        alertTitle: this.props.t("ERROR_MESSAGE"),
    ...
    if (this.state.showRepeat) {
      if (
        this.state.repeatWeekDays === "" ||
        this.state.repeatWeekDays === null ||
        this.state.repeatWeekDays === undefined ||
        this.state.repeatWeekDays > 0
      ) {
        this.setState({
          loading: false,
          alertTitle: this.props.t("ERROR_MESSAGE"),
      ...
      if (
        ...
        this.state.repeatEnd === null ||
        this.state.repeatEnd === undefined ||
        this.state.repeatEnd > 0
      ) {
        this.setState({
          loading: false,
          alertTitle: this.props.t("ERROR_MESSAGE"),
      ...
      if (filterPlan.length > 1) {
        return (
          <select
            required
            name="studentPlanId"
            value={this.state.studentPlanId}
            onChange={this.handleChange}
          >
            {filterPlan.map((planTeste) => (
              <option value={planTeste.id}>{planTeste.plan.nameEnglish}</option>
            ))}
          </select>
        );
  ...
  handlerSubjectEspecific = (boll, value) => {
    this.setState({
      isVisibleText: boll,
      textSpecific: !!boll ? this.state.textSpecific : null,
      focus: value,
    });
  };
  ...
  render() {
    ...
    return (
      <div className="view">
        ...
        >
          <DialogTitle id="alert-dialog-title" onClose={closeAlert}>
            <div
              style={{
                color:
            ...
            <div className="boxWhite">
              <div className="selectDays">
                    ...
                    {languages.map((lang) => (
                      <li>
                        <div>
                          <label
                            className={
                              lingoLanguageId === lang.lingoLanguage.id
                                ? "active"
                                : ""
                            }
                          >
                            {t(lang.lingoLanguage.description)}
                            <input
                              type="radio"
                              value={lang.lingoLanguage.id}
                              name="lingoLanguageId"
                              onClick={(e) =>
                                this.buttonLanguageHandleChange(
                                  e,
                                  lang.lingoLanguage
                                )
                              }
                      ...
                      <li>
                        <select
                          ...
                          onChange={({ target: { value } }) => {
                            value === t("BTN_SPECIFIC")
                              ? this.handlerSubjectEspecific(true, value)
                              : this.handlerSubjectEspecific(false, value);
                          }}
                        >
                  ...
                  <ul style={{ justifyContent: "space-between" }}>
                      ...
                      <select name="scheduledStart" onChange={handleChange}>
                        {timeSlots.map((timeSlots) => (
                          <option
                            ...
                            selected={timeSlots.start === "12:00:00"}
                            key={timeSlots.start}
                          >
                            {timeSlots.start}
                          </option>
                    ...
                    <li style={{ width: "75px" }}>
                      ...
                      >
                        ...
                        {numberOfClassesOptions.map((num) => (
                ...
                <div
                  ...
                  style={{ display: showRepeat ? "block" : "none" }}
                >
                    ...
                    {repeatWeekDays.map((day) => (
                      <li>
                        <button
                          ...
                          value={day.value}
                          onClick={(e) => handleActiveRepeatDay(e)}
                        >
                          {t(day.label)}
                        </button>
                      </li>
                ...
                <div className="buttons">
                  <button className="button-blue" onClick={submitHandle}>
                    {t("BTN_SCHEDULE_HOME")}
                  </button>
                </div>
              </div>
